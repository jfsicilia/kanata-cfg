...
- The `!` and `|` in these names are load-bearing for the sync scripts (`kanata_sync_apps.py` / `kanata_sync_interfaces.py` parse the interface name out of the filename), so their `IFACE_PRIORITY_RE` must keep accepting both characters. `|` here is unrelated to the `key|layer_name` alias pattern in setup.kbd — different files, different parsers, no collision
